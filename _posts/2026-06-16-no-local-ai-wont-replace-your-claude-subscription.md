...
One thing almost nobody talks about: these MoE models don't behave like a normal dense model. There's a tiny **router** that, for every single token, decides which ~10 of the 512 experts actually fire - so only ~3B of the 80B params do the work at each step. Which experts wake up is driven by the input, and it's [semantically conditioned](https://aclanthology.org/2025.findings-emnlp.991.pdf): similar prompts route to similar experts, different phrasing routes elsewhere (someone built a [nice visualizer](https://martinalderson.com/posts/moe-expert-routing-visualization) - for a short prompt ~25% of the experts never fire at all, and it's a *different* 25% each time). So your wording genuinely matters more here than on a dense model. The catch for *local*, though: the router is least reliable on exactly the hard reasoning tokens, and - the big one - [quantizing the model nudges the router itself](https://openreview.net/forum?id=kPgLp47bJf), flipping a few experts right around the top-k cutoff, so your Q4/Q6 copy quietly routes a bit differently than the full-precision original the cloud runs. Stack all that up and you get the streakiness: local can be sharp, sometimes honestly better than Opus on the exact thing, then weirdly dumb a moment later. It's not random - it's the routing (and your quant).
