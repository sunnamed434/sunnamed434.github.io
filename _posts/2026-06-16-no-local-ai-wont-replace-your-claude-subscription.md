...
I also keep a **Windows dual-boot** on the mini PC - out of curiosity I threw a few of the heaviest AAA games at it, and the thing just *flies* (Strix Halo's iGPU is genuinely strong). But I almost never boot into it: I'm just not that into gaming anymore - poking at AI, or honestly just real life, is way more interesting to me right now. The box earns its keep on the Linux side.

On the mini PC I have a search service in Docker (**[SearXNG](https://github.com/searxng/searxng) + an [MCP server](https://github.com/ihor-sokoliuk/mcp-searxng)**, with Google, DuckDuckGo, etc. behind it) so the AI can search the web like Cursor does (and like Claude does too). Later I stacked more MCPs on top - **[context7](https://context7.com/)** for pulling library docs, plus **[exa](https://exa.ai/)** and **[tavily](https://tavily.com/)** for web search.
...
About build quality - mine is a **[BosGame M5](https://www.bosgame.com/products/bosgame-m5-ai-mini-desktop-ryzen-ai-max-395-96gb-128gb-2tb)**. It works, but the build is meh: there was literally a **loose screw rattling around inside** the case. And I'm not the only one - plenty of BosGame owners in the Discord report the exact same loose screw, so it's clearly a recurring QC thing, not a one-off. You almost have to wonder if it's a little warranty trap, too, since the only way to fish it out is to crack the thing open, which is exactly how you might wave goodbye to your warranty. The [Strix Halo wiki](https://strixhalo.wiki/Hardware/PCs) lines up with that: it flags **Beelink** for genuine stability issues and notes **BosGame**'s QC niggles (loose screws, stuck power buttons), even though BosGame is actually one of the more *popular* picks. If I were buying again I'd still look harder at something like the **GMKtec EVO-X2** (or Framework Desktop, Minisforum MS-S1 Max, etc). The brand matters less than the chip inside - and since the Ryzen AI Max+ 395 isn't sold on its own, you just pick whichever prebuilt mini PC ships it with 128 GB.
...
- **[Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled-v2](https://huggingface.co/Jackrong/Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled-v2-GGUF)** - this is the one the "cancel your subscription" crowd loves. It's literally Qwen3.5-27B *distilled from Claude Opus 4.6*. Quick aside on what **distillation** is: you take a big, smart model (the "teacher" - here Opus) and train a smaller model on its outputs/reasoning so the small one learns to imitate it - you get some of the big model's behaviour in a package you can actually run at home. It's noticeably **slower** than the others, though - it's a dense 27B, so every parameter fires on every token, unlike the MoE ones that only light up ~3B at a time. 262k context, Q4_K_M ~16.5 GB.
- **[Qwen3.6-35B-A3B-Claude-4.7-Opus-Reasoning-Distilled-APEX-MTP](https://huggingface.co/mudler/Qwen3.6-35B-A3B-Claude-4.7-Opus-Reasoning-Distilled-APEX-MTP-GGUF)** - this is where it gets *usable*. The genuinely good local models right now are **MTP** ones (multi-token prediction). 35B-A3B MoE, distilled from Opus 4.7, and it hits **60+ tokens/s**. That's the speed that makes local feel real. The **[APEX](https://github.com/mudler/apex-quant)** part (Adaptive Precision for EXpert Models) is a smarter, MoE-aware twist on the quantization from above: instead of squashing every weight to the same bit-width, it compresses the experts that rarely fire the hardest and keeps the always-on bits (the shared expert, attention) at higher precision - so you save memory exactly where it costs the least quality.
...
Now a quirk almost nobody talks about: *which* ~3B fire isn't fixed. There's a tiny **router** that, for every single token, picks which ~10 of the 512 experts light up, and that choice is driven by the input - it's [semantically conditioned](https://aclanthology.org/2025.findings-emnlp.991.pdf): similar prompts route to similar experts, different phrasing routes elsewhere (someone built a [nice visualizer](https://martinalderson.com/posts/moe-expert-routing-visualization) - for a short prompt ~25% of the experts never fire at all, and it's a *different* 25% each time). So your wording genuinely matters more here than on a dense model. The catch for *local*, though: from what I've read the router is shakiest on exactly the hard reasoning tokens, and - the big one - [quantizing the model nudges the router itself](https://openreview.net/forum?id=kPgLp47bJf), flipping a few experts right around the top-k cutoff, so your Q4/Q6 copy quietly routes a bit differently than the full-precision original the cloud runs. That's the streakiness: local can be sharp, sometimes honestly better than Opus on the exact thing, then weirdly dumb a moment later. It's not random - it's the routing (and your quant).
...
- **No quota, nothing ticking down.** This is honestly the one thing about Claude that genuinely gets under my skin. You're not up against one limit but *two* - a [rolling few-hour cap **and** a separate weekly one](https://support.anthropic.com/en/articles/11647753-understanding-usage-and-length-limits) - and either can wall you off mid-task. So you catch yourself rushing, or shaping your whole day's work *around* the limits instead of around the actual problem, always half-aware of the meter ticking. And it's not a Claude-only thing - [ChatGPT runs the same playbook](https://openai.com/chatgpt/pricing) (Plus is roughly 160 messages per few hours, plus a weekly cap on the thinking model). Locally? None of it. You sit and grind on one thing for hours, and the only limits left are your internet and your electricity bill.
...
- **Uncensored models are a genuine time-saver, not just an edgy gimmick.** I lean on abliterated ones a lot, and the reason is boring and practical: they save tokens. A safety-tuned model loves to hedge - disclaimers, "I'm not a lawyer, but...", little moral preambles - and every so often it flat-out refuses something completely normal, so you burn a whole round-trip re-asking or rephrasing, sometimes padding the prompt with jailbreak scaffolding that eats into your context. Abliteration deletes the "refusal direction" straight out of the weights - it turns out refusal is [basically a single direction in the model](https://www.lesswrong.com/posts/jGuXSZgv6qfdhMCuJ/refusal-in-llms-is-mediated-by-a-single-direction), so it can be surgically removed without retraining - and the model then *just answers*: no hedging, no moralizing, no wasted output. The paid side is the opposite: there's a whole safety apparatus bolted around the model. Anthropic literally runs [separate "constitutional classifiers"](https://www.anthropic.com/research/next-generation-constitutional-classifiers) that sit in front of and behind the model, scanning your input and your output and able to refuse or block a request on their own - a whole extra layer on top of the model's own training. Local + abliterated keeps all of that out of the way - less time, fewer tokens, less context spent fighting the model instead of using it.
